...
$pdf->ChapterBody("Total Revenue Today: TZS " . number_format($totalRevenueToday, 0, '.', ','));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Week: TZS " . number_format($totalRevenueThisWeek, 0, '.', ','));
$pdf->Ln();

// Calculate total revenue for this month
$currentMonthStart = date('Y-m-01');

// SQL query to sum up total revenue for the current month
$totalRevenueThisMonthQuery = "
    SELECT IFNULL(SUM(item_price * quantity), 0) AS total_revenue
    FROM (
        SELECT bi.item_id, 
...
$totalRevenueThisMonth = $totalRevenueThisMonthRow['total_revenue'] ?? 0; // Ensure a default value of 0

// Output or use the total revenue as needed
echo "Total Revenue for this Month: " . number_format($totalRevenueThisMonth, 2) . " TZS";


// Monthly Report
$pdf->ChapterTitle('Monthly Report');
$pdf->ChapterBody("Date Range: " . $currentMonthStart . " to " . date('Y-m-t'));
$pdf->ChapterBody("Total Revenue This Month: TZS " . number_format($totalRevenueThisMonth, 0, '.', ','));
$pdf->Ln();
...
$pdf->ChapterBody("Total Revenue This Year: TZS " . number_format($totalRevenueThisYear, 0, '.', ','));
$pdf->Ln();
...
$header = array('Date','Day' , 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($dailyCategoryRevenue)) {
    $data[] = array($row['date'], $row['day'], number_format($row['daily_category_revenue'], 0, '.', ','));
}
...
$header = array('Date','Week' , 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($weeklyCategoryRevenue)) {
    $data[] = array($row['year'], $row['week'], number_format($row['weekly_category_revenue'], 0, '.', ','));
}
$pdf->CustomTableThreeColumn($header, $data);
...
$header = array('Date','Month' , 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($monthlyCategoryRevenue)) {
    $data[] = array($row['year'], $row['month'], number_format($row['monthly_category_revenue'], 0, '.', ','));
}
$pdf->CustomTableThreeColumn($header, $data);
...
$header = array('Date','Day' , 'Item Category', 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($dailyCategoryRevenue)) {
    $data[] = array($row['date'], $row['day'], $row['item_category'], number_format($row['daily_category_revenue'], 0, '.', ','));
}
$pdf->CustomTableFourColumn($header, $data);
...
$header = array('Date','Week' , 'Item Category', 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($weeklyCategoryRevenue)) {
    $data[] = array($row['year'], $row['week'], $row['item_category'], number_format($row['weekly_category_revenue'], 0, '.', ','));
}
$pdf->CustomTableFourColumn($header, $data);
$pdf->Ln();
...
$header = array('Date','Month' , 'Item Category', 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($monthlyCategoryRevenue)) {
    $data[] = array($row['year'], $row['month'], $row['item_category'], number_format($row['monthly_category_revenue'], 0, '.', ','));
}
$pdf->CustomTableFourColumn($header, $data);
...
$header = array('Date', 'Item Category', 'Revenue (TZS)');
$data = array();
while ($row = mysqli_fetch_assoc($yearlyCategoryRevenue)) {
    $data[] = array($row['year'], $row['item_category'], number_format($row['yearly_category_revenue'], 0, '.', ','));
}
$pdf->CustomTableThreeColumn($header, $data);
