...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $baseUnitQuantity = $_POST["baseUnitQuantity"];
    $conversionRatio = $_POST["conversionRatio"];
    $pricePerBaseUnit = $_POST["pricePerBaseUnit"];
    $pricePerSubUnit = $_POST["pricePerSubUnit"];
    $conn = $link;

    // Prepare the SQL query to check if the item already exists (optional)
    $check_query = "SELECT ItemName FROM stock WHERE ItemName = ?";
    ...
    if ($check_result->num_rows > 0) {
        $message = "A stock item with the same name already exists.<br>Please try again with a different name.";
        $iconClass = "fa-times-circle";
        $cardClass = "alert-danger";
        $bgColor = "#FFA7A7"; // Custom background color for error
    } else {
        // Prepare the SQL query for insertion
        $insert_query = "INSERT INTO stock (ItemName, BaseUnitQuantity, ConversionRatio, PricePerBaseUnit, PricePerSubUnit) 
                        VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);

        // Bind the parameters
        $stmt->bind_param("siidd", $itemName, $baseUnitQuantity, $conversionRatio, $pricePerBaseUnit, $pricePerSubUnit);
...
<body>
    ...
    <script>
        ...
        function showPopup() {
            ...
            var countdownInterval = setInterval(function() {
                ...
                if (i <= 0) {
                    ...
                    window.location.href = "createStock.php";
                }
        ...
        function hidePopup() {
            ...
            setTimeout(function () {
                window.location.href = "createStock.php"; // Replace with your desired URL
